...
Plugin URI: https://github.com/kevinlopez14/gravity-forms-automatic-export-to-csv
Description: Automatically send an email containing a CSV export of your Gravity Form entries on a schedule.
Version: 0.3.2
Author: Kevin Lopez
Author URI: https://github.com/kevinlopez14/
...
class GravityFormsAutomaticCSVExport {
	...
	*/
	public function gforms_automated_export() {
		...
		$allmails = explode(",", $email_address);
		...
		foreach ($allmails as $key ) {
			wp_mail( $key , 'Automatic Form Export', 'CSV export is attached to this message', $headers, $attachments);
		}
	}
	...
	public static function start_automated_export( $form, $offset = 0, $export_id = '' ) {
		...
		$start_date = empty( $_POST['export_date_start'] ) ? '' : gmdate( $_POST['export_date_start'] . ' 00:00:00', time() );
		$end_date   = empty( $_POST['export_date_end'] ) ? '' : gmdate( $_POST['export_date_end'] . ' 23:59:59', time());
